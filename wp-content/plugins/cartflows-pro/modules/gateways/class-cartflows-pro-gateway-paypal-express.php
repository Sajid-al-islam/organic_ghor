...
class Cartflows_Pro_Gateway_Paypal_Express extends Cartflows_Pro_Paypal_Gateway_helper {
							...
							'<?php echo esc_attr( $this->get_woo_payer_id() ); ?>',
							{
								...
								click: function () {
									...
									var postData = {
										...
										security: security: <?php echo sanitize_key( wp_create_nonce( 'cartflows_front_create_ppec_paypal_checkout_token_nonce' ) ); ?>
									};
	...
	public function generate_express_checkout_token() {
		...
		if ( $is_valid_order ) {
			...
			wcf()->logger->log( 'Generate express checkout token' );
				...
				if ( isset( $data['PAYMENTREQUEST_0_CUSTOM'] ) ) {
					...
					if ( isset( $get_custom_attrs->order_id ) ) {
						...
						try {
							...
							wcf()->logger->log( 'Details Below: ' . wcf_print_r( $e->getMessage(), true ) );
						}
					}
				}
			}
		...
		switch ( $_GET['action'] ) {
			...
			case 'cartflows_paypal_return':
				...
				if ( isset( $_GET['token'] ) && ! empty( $_GET['token'] ) ) {
					...
					wcf()->logger->log( 'Express checkout token return request' );
					...
					if ( $offer_product['total'] > 0 ) {
						...
						wcf()->logger->log( 'Express $response_checkout checkout token charge' );
	...
	public function process_offer_refund( $order, $offer_data ) {
		...
		if ( ! is_null( $refund_amount ) && class_exists( 'WC_Gateway_Paypal' ) ) {
			...
			if ( isset( $available_gateways['paypal'] ) ) {
				...
				} else { // Ignoring naming conventions for response variables.
					switch ( strtolower( $result->ACK ) ) { //phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase

						case 'success':
						case 'successwithwarning':
							$response_id = $result->REFUNDTRANSACTIONID; //phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
					}
				}
				if ( isset( $result->L_LONGMESSAGE0 ) ) { //phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
					wcf()->logger->log(
						...
						wp_json_encode( $result->L_LONGMESSAGE0 ) //phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
					);
				}
			}
	...
	public function offer_refund_request_data( $request, $order, $amount ) {
		...
		if ( isset( $_POST['cartflows_refund'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Missing
			...
			if ( $this->key === $payment_method ) {
				...
				if ( isset( $_POST['transaction_id'] ) && ! empty( $_POST['transaction_id'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Missing

					$request['TRANSACTIONID'] = sanitize_text_field( wp_unslash( $_POST['transaction_id'] ) ); //phpcs:ignore WordPress.Security.NonceVerification.Missing
